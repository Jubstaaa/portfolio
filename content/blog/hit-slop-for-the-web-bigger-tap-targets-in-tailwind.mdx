...
<Note>
    ...
    forgiving click area too. Wrap them in `pointer-coarse:` when you want the
    slop only on touch devices.
</Note>
...
<Warn>
    An ancestor with `overflow: hidden` silently clips the slop — cards, modals
    and scroll containers are the usual suspects. Toggle `hit-slop-debug` and
    look.
</Warn>
